...
namespace QSB2.DeathSync;

[HarmonyPatch]
public class DeathPatches()// : QPatch(QPatchWhen.OnQObjectsCreated)
{
	...
	public static bool PauseMenuManager_OnActivateMenu(PauseMenuManager __instance)
	{
		...
		{
			bool flag = !PlayerState.IsSleepingAtCampfire() && !PlayerState.IsGrabbedByGhost() && !NetworkManager.LocalConnection.QPlayer.IsDead;
...
[MessagePackObject]
public class PlayerDeathMessage : QObjectMessage<QPlayer>
{
    [Key(1)]
    public DeathType DeathType;

    public override void OnReceive(QPlayer qObject, int from, int to)
    {
	    qObject.IsDead = true;
    }
}