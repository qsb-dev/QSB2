...
[HarmonyPatch]
public class ProfilerPatch() : QPatch(QPatchWhen.Immediately)
{
    // private readonly static List<ProfilerZone> _zones = new();
    
    public static IEnumerator EndOfFrameCo()
    {
        while (true)
        {
    ...
    private static string FriendlyName(MethodBase @this) => $"{@this.DeclaringType.Name}.{@this.Name}";
    ...
    public static IEnumerable<MethodBase> TargetMethods()
    {
        foreach (var type in Assembly.GetExecutingAssembly().GetTypes())
        {
            ...
            foreach (var method in type.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly))
            {
                ...
                Logger.Log($"[profiler] profiling {FriendlyName(method)}");
                yield return method;
            }
        }
    ...
    [HarmonyPrefix]
    public static void Prefix(MethodBase __originalMethod, out ProfilerZone __state)
    {
        __state = Profiler.BeginZone(FriendlyName(__originalMethod));
    }

    [HarmonyPostfix]
    public static void Postfix(ProfilerZone __state)
    {
        __state.Dispose();
    }
}
...
public readonly struct ProfilerZone : IDisposable
{
    ...
    internal ProfilerZone(PInvoke.TracyCZoneCtx context)
    {
        Context = context;
    }
